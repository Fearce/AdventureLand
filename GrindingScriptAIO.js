//Respawning
/* Respawn Interval - Setup */
setInterval(function()
{
  if(character.rip == true){
    let respawnX = character.real_x; // respawnX = X Coordinate from your character where this script is running
    let respawnY = character.real_y; // respawnY = Y Coordinate from your character where this script is running
    let respawnMap = character.map; // respawnMap = Map from your character where this script is running
    respawn(); //Respawn the character if he is dead
    smart_move(respawnX, respawnY, respawnMap); //Smart_Move to last X, Y, Map Coordinates
  }
}, 12000);
...
//Merchant send
	...
	var receiver="FearceM";
	var gold="100000";

	if(get_player(receiver) && send_gold(receiver,gold)){
		for(var i = 2; i <= character.items.length; i++){
				send_item(receiver,i,1000);
			}
...
var mtype2 = 'squig'; //Monster Type of the enemy you want to attack if you can't find the first
...
var purchase_pots = true; //Enable Potion Purchasing = true, Disable Potion Purchasing = false
var buy_hp = true; //Allow HP Pot Purchasing = true, Disallow HP Pot Purchasing = false
